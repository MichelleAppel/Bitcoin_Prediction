...
# batch_size * sequence length * data dimensions

# Features = data_dim

# L1 of L2 loss function
...
with tf.variable_scope("lstm"):
    lstm = tf.contrib.rnn.LSTMCell(num_units=lstm_units, forget_bias=forget_gate_bias)
...
with tf.variable_scope("softmax"):
    ...
    conc = tf.concat(output, 1, name='concat')
    output = tf.reshape(conc, [-1, lstm_units])
